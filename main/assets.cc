...
bool Assets::Download(std::string url, std::function<void(int progress, size_t speed)> progress_callback) {
    ...
    // Rimuovi la mappatura della partizione assets corrente
    UnApplyPartition();

    // Scarica il nuovo file delle risorse
    ...
    // Definisci la dimensione del settore (tipicamente 4KB su ESP32)
    const size_t SECTOR_SIZE = esp_partition_get_main_flash_sector_size();
    
    // Calcola il numero di settori da cancellare
    size_t sectors_to_erase = (content_length + SECTOR_SIZE - 1) / SECTOR_SIZE; // arrotondamento per eccesso
    ...
    // Scrivi il nuovo file assets nella partizione: cancella e scrivi progressivamente
    ...
    while (true) {
        ...
        while (current_sector < needed_sectors) {
            ...
            // Assicurati che l'intervallo cancellato non superi la dimensione della partizione
        ...
        // Scrivi i dati nella partizione
        ...
        if (esp_timer_get_time() - last_calc_time >= 1000000 || total_written == content_length || ret == 0) {
            ...
            recent_written = 0; // Reimposta il conteggio dei byte scritti recentemente
        }
    }
    ...
    // Reinizializza la partizione assets
