...
void OggDemuxer::Reset()
{
    ...
    ctx_.seg_index = 0;
    ctx_.data_offset = 0;
    ctx_.bytes_needed = 4;          // Richiede 4 byte per "OggS"
    ...
    // 清空缓冲区数据
    memset(ctx_.header, 0, sizeof(ctx_.header)); // Svuota i buffer
    memset(ctx_.seg_table, 0, sizeof(ctx_.seg_table)); // Svuota i buffer
    memset(ctx_.packet_buf, 0, sizeof(ctx_.packet_buf)); // Svuota i buffer
}
    ...
    while (processed < size) {
        switch (state_) {
          case ParseState::FIND_PAGE: {
            // 寻找页头"OggS"
            if (ctx_.bytes_needed < 4) { // Cerca l'intestazione di pagina "OggS"
                // 处理不完整的"OggS"匹配（跨数据块）
                size_t to_copy = std::min(size - processed, ctx_.bytes_needed); // Gestisce un match parziale di "OggS" (attraverso più blocchi)
                ...
                if (ctx_.bytes_needed == 0) {
                    // 检查是否匹配"OggS"
                    if (memcmp(ctx_.header, "OggS", 4) == 0) { // Verifica se corrisponde a "OggS"
                        state_ = ParseState::PARSE_HEADER;
                        ctx_.data_offset = 4;
                        ctx_.bytes_needed = 27 - 4;  // 还需要23字节完成页头
                    } else {
                        // 匹配失败，滑动1字节继续匹配
                        memmove(ctx_.header, ctx_.header + 1, 3); // Match fallito, scorrere di 1 byte e continuare la ricerca
                        ctx_.bytes_needed = 1;
                    }
                } else {
                    // 数据不足，等待更多数据
                    return processed; // Dati insufficienti, attendi altri dati
                }
            } else if (ctx_.bytes_needed == 4) {
                // 在数据块中查找完整的"OggS"
                bool found = false; // Cerca una sequenza completa "OggS" all'interno del blocco
                size_t i = 0;
                size_t remaining = size - processed;
                
                // 搜索"OggS"
                for (; i + 4 <= remaining; i++) { // Ricerca di "OggS"
                ...
                if (found) {
                    // 找到"OggS"，跳过已搜索的字节
                    processed += i; // Trovato "OggS", saltare i byte già scansionati
                    
                    // 不记录找到的"OggS"，无必要
                    // Non memorizzare l'intestazione trovata (non necessario)
                    processed += 4;
                    ...
                    size_t partial_len = remaining - i; // "OggS" non trovato, salva eventuale parziale corrispondenza
                    if (partial_len > 0) {
                    ...
                    return processed;  // Ritorna il numero di byte processati
                }
            } else {
          ...
          case ParseState::PARSE_HEADER: {
            ...
            if (available < ctx_.bytes_needed) {
                // 数据不足，复制可用的部分
                memcpy(ctx_.header + ctx_.data_offset,  // Dati insufficienti, copiare la parte disponibile
                ...
                // 有足够的数据完成页头
                size_t to_copy = ctx_.bytes_needed; // Disponibili dati sufficienti per completare l'intestazione
                ...
                // 验证页头
                if (ctx_.header[4] != 0) { // Verifica l'intestazione
                ...
                if (ctx_.seg_count > 0 && ctx_.seg_count <= 255) {
                    ...
                    // 没有段，直接跳到下一个页面
                    state_ = ParseState::FIND_PAGE; // Nessun segmento, passare alla pagina successiva
                    ctx_.bytes_needed = 4;
                    ctx_.data_offset = 0;
                } else {
          ...
          case ParseState::PARSE_SEGMENTS: {
            ...
            if (available < ctx_.bytes_needed) {
                memcpy(ctx_.seg_table + ctx_.data_offset,  // Copia i dati
                        data + processed, available);
                ...
                // 计算数据体总大小
                ctx_.body_size = 0; // Calcola la dimensione totale del corpo dei dati
          ...
          case ParseState::PARSE_DATA: {
            while (ctx_.seg_index < ctx_.seg_count && processed < size) {
                ...
                // 检查段数据是否已经部分读取
                if (ctx_.seg_remaining > 0) { // Controlla se il segmento è già stato parzialmente letto
                ...
                // 检查缓冲区是否足够
                if (ctx_.packet_len + seg_len > sizeof(ctx_.packet_buf)) { // Verifica che il buffer sia sufficiente
                ...
                // 复制数据
                size_t to_copy = std::min(size - processed, (size_t)seg_len); // Copia i dati
                ...
                // 检查段是否完整
                if (ctx_.seg_remaining > 0) { // Controlla se il segmento è completo
                    // 段不完整，等待更多数据
                    return processed;
                }
                
                // 段完整
                bool seg_continued = (ctx_.seg_table[ctx_.seg_index] == 255); // Segmento completo
                
                if (!seg_continued) {
                    // 包结束
                    if (ctx_.packet_len) { // Fine del pacchetto
                        ...
                        if (opus_info_.head_seen && opus_info_.tags_seen) {
                            ...
                            ESP_LOGW(TAG, "Il contenitore Ogg non contiene OpusHead/OpusTags, scartato"); // Il contenitore Ogg non contiene OpusHead/OpusTags, scartato
                        }
                    }
                    ctx_.packet_len = 0;
                    ctx_.packet_continued = false;
            ...
            if (ctx_.seg_index == ctx_.seg_count) {
                // 检查是否所有数据体都已读取
                if (ctx_.body_offset < ctx_.body_size) { // Verifica se tutto il corpo dei dati è stato letto
                    ESP_LOGW(TAG, "数据体不完整: %zu/%zu", 
                            ctx_.body_offset, ctx_.body_size);
                }
                
                // 如果包跨页，保持packet_len和packet_continued
                if (!ctx_.packet_continued) { // Se il pacchetto è su più pagine, mantenere packet_len e packet_continued
                    ctx_.packet_len = 0;
                }
                
                // 进入下一页面
                state_ = ParseState::FIND_PAGE; // Passa alla pagina successiva
                ctx_.bytes_needed = 4;
                ctx_.data_offset = 0;
            }
            break;
